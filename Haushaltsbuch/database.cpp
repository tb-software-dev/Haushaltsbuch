...
// Destruktor: Schließen der Datenbank-Verbindung falls sie bereits offen ist
Database::~Database()
{
	if (m_db.isOpen())
	{
...
bool Database::initialize()
{
	if (!m_db.open())
	{
		qDebug() << "Fehler beim Öffnen der Datenbank:" << m_db.lastError().text();
		return false;
	}
...
bool Database::createTransactionsTable()
{
	...
	return query.exec(
		...
		"account_id INTEGER, "						// Verknüpfung zur Account-Tabelle
		"FOREIGN KEY (account_id) REFERENCES accounts(id))");
}

// Erstellt die Tabelle für die Budgets
...
// Erstellt die Tabelle für die Konten
...
bool Database::saveTransaction(Transaction& transaction)
{
	...
	// Platzhalter mit den Werten von der Transaktion füllen
	...
	if (query.exec())
	{
		// ID der neu eingefügten Transaktion setzen
...
bool Database::deleteTransaction(int id)
{
	...
	qDebug() << "Fehler beim Löschen der Transaktion:" << query.lastError().text();
	return false;
}

// Gibt alle Transaktionen in einem bestimmten Zeitraum zurück
